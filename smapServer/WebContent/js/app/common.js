...
function downloadFile(url) {
	...
	url += addCacheBuster(url);
	var $iframe = $("<iframe style='display: none;'></iframe>").attr("src", url);
	$("body").append($iframe);
	// Check for errors allow 5 seconds for an error to be returned
	setTimeout(function () {
		downloadFileErrorCheck($iframe);
	}, 5000);
}

// Show an error generated by file download
function downloadFileErrorCheck($iframe) {

	var msg;

	if(!$iframe || $iframe.length === 0) {
		$iframe = $("iframe").last();		// Called without an iframe, use the most recent one
	}
	try {
		msg = $iframe.contents().find('body').html();
	} catch (e) {
		return;			// The response cannot be read, assume the download worked
	}

	if(handleLogout(msg)) {		// handleLogout matches on the raw html of a container error page
		var text = errorMessageText(msg);
		if(text.length > 0) {
			alert(text);
		}
	}
...
/*
 * Convert the body of a failed file download into text that can be shown in an alert
 *
 * The body is either a message from the application, prefixed with "Error:", or, when the
 *  request did not reach the application, an html error page generated by the container.
 *  Show the heading of an error page rather than its markup, which is neither readable nor
 *  safe to pass on as html
 */
function errorMessageText(msg) {
	...
	if(text.indexOf("<") >= 0) {
		try {
			// DOMParser is used rather than jquery so that nothing in the response is run or loaded
			var doc = new DOMParser().parseFromString(text, "text/html");
			var heading = doc.querySelector("h1, h2, h3");
			text = heading ? heading.textContent : doc.body.textContent;
		} catch (e) {
			text = text.replace(/<[^>]*>/g, " ");	// DOMParser is unavailable, drop the tags
		}
	}

	text = text.replace(/\s+/g, " ").trim();
	if(text.indexOf("Error:") === 0) {
		text = text.substring(6).trim();		// Jump over "Error:"
	}
	return text;
}
...
function addGeomPickList(sMeta) {
	...
	if(sMeta && sMeta.forms) {
		for(i = 0; i < sMeta.forms.length; i++) {
			...
			k[++idx] = h[++idx] = '<div class="exportcontrol showshape showosm mb-2">';
			k[++idx] = h[++idx] = '<label class="form-label">' + htmlEncode(theForm.form) + '</label>';
			h[++idx] = '<select class="geomSelect form-select" id="geomForm_' + theForm.f_id;            // export only
			k[++idx] = '<select class="geomSelect form-select" id="geomSettingsForm_' + theForm.f_id;    // Settings only
