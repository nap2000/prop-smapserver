...
#	pgsql2shp -f $4/$2 -u ws -P ws1234 $1 "$3"
	...
	ogr2ogr -f "ESRI Shapefile" $4/$2 PG:"dbname=$1 host=$DBHOST user=ws password=ws1234" -sql "$3"
	zip -rj $4.zip $4
fi
if [ "$5" = "media" ]
then
...
then
	...
	ogr2ogr -f "KML" $4.kml PG:"dbname=$1 host=$DBHOST user=ws password=ws1234" -sql "$3"
	zip -rj $4.zip $4.kml
fi
if [ "$5" = "vrt" ] || [ "$5" = "stata" ]
then
	...
	psql $1 -h $DBHOST -U ws << EOF > $4/$2.csv
...
then
	...
	psql $1 -h $DBHOST -U ws << EOF > $4/$2.nb
COPY ($3) TO STDOUT WITH CSV HEADER
EOF
...
then
        ...
        psql $1 -h $DBHOST -U ws << EOF > $4.csv
COPY ($3) TO STDOUT WITH CSV HEADER
EOF
